class DimensionalVector:
    ...
    def _dimension_check(self,d):
        #Check if d is a positive integer.
        if not isinstance(d, int) or d <= 0:
            raise ValueError("Dimension must be a positive integer")
    def _length_check(self, other):
        #Check if dimensions of self and other are of equal length for operation.
        if len(self) != len(other):
            raise ValueError("Dimensions must agree for arithmetic operations")
